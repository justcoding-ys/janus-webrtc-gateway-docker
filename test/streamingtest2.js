...
if (window.location.protocol === 'http:')
	server = "http://" + window.location.hostname + "/rest";
else
	server = "https://" + window.location.hostname + "/rest";
...
// var selectedStream = 9620;
const urlParams = new URLSearchParams(window.location.search);
// var selectedStream = urlParams.get('id');
var tid = Math.random().toString(36).substring(8);

var rtp_forward = function(sid, pid, pubid) {
	$.ajax({
		type : 'POST',
		url : server + '/' + sid + '/' + pid,
		data : JSON.stringify({
		...
		success : function(resp) {
			if (resp.janus === "success") {
				console.log(resp.plugindata);
				console.log('rtp forward success');
			}
			else
				console.log('Failed rtp_forward: ' + resp.error);
		}
	});
}

var get_rtp_forward_list = function(sid, pid, pubid) {
	$.ajax({
		// Get RTP forward list
		type : 'POST',
		url : server + '/' + sid + '/' + pid,
		data : JSON.stringify({
			janus : 'message',
		...
		success : function(resp) {
			if (resp.janus === "success") {
				forwarders = resp.plugindata.data.rtp_forwarders;
				console.log("!!!!!!!");
				console.log(resp.plugindata.data);
				console.log("!!!!!!!");
				var bfind = false;
				for (var i = 0; i < forwarders.length; i++) {
					if (forwarders[i].publisher_id === pubid) {
						console.log('Already forwarded .. ');
						bfind = true;
						break;
					}
				}
				if (!bfind)
					rtp_forward(sid, pid, pubid);
			}
			else
				console.log('Failed get_rtp_forward_list: ' + resp.error);
		}
	});
}

var get_users_in_room = function(sid, pid) {
	$.ajax({
		type : 'POST',
		url : server + '/' + sid + '/' + pid,
		data : JSON.stringify({
		...
		success : function(resp) {
			console.log(resp.plugindata);
			if (resp.janus === "success") {
				var users = resp.plugindata.data.participants;
				if (users.length > 0 && users[0].publisher) {
					get_rtp_forward_list(sid, pid, users[0].id);
				} else {
					console.log('Not found publisher..');
					alert('먼저 OBS로 videoroom 에 방송을 시작해 주시기 바랍니다.');
				}
			} else
				console.log('Failed get_users_in_room: ' + resp.error.reason);
		}
	})
}

var attach_plugin = function(sid, pname) {
	$.ajax({
		type : 'POST',
		url : server + '/' + sid,
		data : JSON.stringify({
		...
		success : function(resp) {
			if (resp.janus === "success")
				get_users_in_room(sid, resp.data.id);
			else
				console.log('Failed attach plugin: ' + resp.error);
		}
	});
}

var start_forward = function() {
	// Create session
	$.ajax({
		type : 'POST',
		url : server,
		...
		success : function(resp) {
			if (resp.janus === "success")
				attach_plugin(resp.data.id, 'janus.plugin.videoroom');
			else
				console.log('Failed create session: ' + resp.error);
		}
	});
}

$(document).ready(function() {
	$('#forward').on('click', start_forward);
